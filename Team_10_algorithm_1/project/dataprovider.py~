...
def get_trip_details(limit):
    trip_details_dictionary = {}
    connection_object = dbconnection.open_db_connection()
    cursor = connection_object.cursor()
    temp="select * from original_trip_data where pickup_datetime >" + "STR_TO_DATE("+"'"+str(limit) +"', '%Y-%m-%d %H:%i:%s')"+ " order by pickup_datetime"
    cursor.execute(temp)
    first_record = cursor.fetchone()
    limit = first_record[3]+ timedelta(minutes=5)
    cursor.execute("select * from original_trip_data where passenger_count between 1 and 4 order by pickup_datetime")
    if(cursor==None):
        return True,limit,trip_details_dictionary
    ...
    return False,limit,trip_details_dictionary
